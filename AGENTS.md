...
`pnpm lint && pnpm typecheck && pnpm test && pnpm build`, plus `pnpm generate`
leaving a clean `git diff` — the same five gates CI runs. `pnpm format` writes
what `pnpm lint` checks.
...
Everything that consumes the TypeScript _API_ — tsup's dts emit,
